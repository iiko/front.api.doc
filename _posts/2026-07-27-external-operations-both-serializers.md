...
Методы с ([кроссплатформенным сериализатором]({{ site.baseurl }}{% post_url 2026-05-04-external-operations-serializer-change %})) переименованы в `RegisterExternalOperationCrossPlatform<TRequest, TResponse>` и `CallExternalOperationCrossPlatform<TRequest, TResponse>`.